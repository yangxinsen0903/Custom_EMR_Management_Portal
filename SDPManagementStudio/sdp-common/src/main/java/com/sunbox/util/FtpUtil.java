...
public class FtpUtil {
     ...
     * 维护FTPClient实例
     */
    private final static LinkedBlockingQueue<FTPSClient> FTP_CLIENT_QUEUE = new LinkedBlockingQueue<>();
    ...
    public static boolean makeDirectory(FtpConfig ftpConfig, String remotePath, String makePath) {
        try {
            FTPSClient ftpClient = connectClient(ftpConfig);
     ...
     * @throws IOException
     */
    private static FTPSClient connectClient(FtpConfig ftpConfig) throws IOException {
     ...
     * @return 获取client对象
     */
    private static FTPSClient getClient() {
        FTPSClient ftpClient = FTP_CLIENT_QUEUE.poll();
        if (ftpClient != null) {
            return ftpClient;
        }
    ...
    private static void offer(FTPSClient ftpClient) {
