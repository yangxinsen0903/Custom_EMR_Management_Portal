...
public class SortUtil {
     ...
     * 排序字符串拼接
     * @param keyMap 要排序的集合
     * @return key=value&key=value 升序
     */
    public static String sortListToStr(Map<String, Object> params){
        ...
        for (int i = 0; i < list.size(); i++) {
            if (params.get(list.get(i)) != null && params.get(list.get(i)).toString().length() > 0){
                if (i > 0) {
                    shastr = shastr + "&" + list.get(i) + "=" + params.get(list.get(i));
                } else {
                    shastr = shastr + list.get(i) + "=" + params.get(list.get(i));
                }
            }
