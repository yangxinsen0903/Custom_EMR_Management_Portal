...
begin
    ...
    set var_table_name = 'info_ambari_config_group';
    set var_table_column = 'sdp_group_name';
    set column_type = 'varchar(100)';
    set comment_value = 'sdp实例组名称';
    set table_count = 1 ;
    while table_count > 0 do
            ...
            IF EXISTS (SELECT * FROM information_schema.columns WHERE table_schema = var_db_name and table_name = @table_name ) THEN
                IF NOT EXISTS (SELECT * FROM information_schema.columns WHERE table_schema = var_db_name and table_name = @table_name AND column_name = var_table_column ) THEN
                    set @statement = CONCAT('alter table ',var_db_name, '.',@table_name, ' add column ',var_table_column,' ',column_type,' DEFAULT 0 COMMENT "',comment_value,'"');
