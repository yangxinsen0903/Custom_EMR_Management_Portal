...
begin
    ...
    set var_table_name = 'ambari_component_layout';
    set var_table_column = 'stack_code';
    set column_type = 'varchar(48)';
    set comment_value = 'Stack版本,默认是SDP-1.0';
    set table_count = 1 ;
    while table_count > 0 do
            ...
            IF EXISTS (SELECT * FROM information_schema.columns WHERE table_schema = var_db_name and table_name = @table_name ) THEN
                IF NOT EXISTS (SELECT * FROM information_schema.columns WHERE table_schema = var_db_name and table_name = @table_name AND column_name = var_table_column ) THEN
                    set @statement = CONCAT('alter table ',var_db_name, '.',@table_name, ' add column ',var_table_column,' ',column_type,' DEFAULT "SDP-1.0" COMMENT "',comment_value,'"');
