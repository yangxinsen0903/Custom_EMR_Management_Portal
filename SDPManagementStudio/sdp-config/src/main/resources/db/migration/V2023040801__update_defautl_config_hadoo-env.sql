DELETE FROM ambari_config_item WHERE config_type_code = 'hadoop-env' and `key` = 'content';
...
INSERT INTO ambari_config_item (stack_code,service_code,component_code,config_type_code,`key`,value,is_content_prop,is_dynamic,dynamic_type,item_type,state,created_by,created_time,updated_by,updated_time) VALUES
    ('SDP-1.0','HDFS','NULL','hadoop-env','content','
      ...
      # remote nodes.
      export HADOOP_OPTS="-Dorg.wildfly.openssl.path=/usr/lib/x86_64-linux-gnu ${HADOOP_OPTS}"
      ...
      {% endif %}',1,0,NULL,'NON_HA','VALID','system',now(),'system',now());
...
INSERT INTO ambari_config_item (stack_code,service_code,component_code,config_type_code,`key`,value,is_content_prop,is_dynamic,dynamic_type,item_type,state,created_by,created_time,updated_by,updated_time) VALUES
    ('SDP-1.0','HDFS','NULL','hadoop-env','content','
      ...
      {% if hadoop_zkfc_opts is defined %}
      export HDFS_ZKFC_OPTS="{{hadoop_zkfc_opts}} $HDFS_ZKFC_OPTS"
      {% endif %}',1,0,NULL,'HA','VALID','system',now(),'system',now());
