...
call create_index('azure_cleaned_vms_record', 'idx_cluster_name', ' (cluster_name)' );

call create_index('azure_cleaned_vms_record', 'idx_vm_name', ' (vm_name)' );
...
begin
    ...
    set var_table_name = 'azure_cleaned_vms_record';
    set var_table_column = 'invoke_count';
    set column_type = 'int default 1';
    set comment_value = '调用接口次数';
    set table_count = 1 ;
...
begin
    ...
    set var_db_name = (select database());
    set var_table_name = 'azure_cleaned_vms_record';
    set var_table_column = 'vms_del_response';
    set column_type = 'text';
    set comment_value = '删除虚拟机响应值';
    set table_count = 1 ;
    while table_count > 0 do
            set @table_name = var_table_name;
            if table_count <> 1 then
