 ...
 */
//@EnableAsync
//@Configuration("sendDeleteReq")
public class SendDeleteReqAsyncConfig implements AsyncConfigurer, BaseCommonInterFace {

    private ThreadPoolTaskExecutor executorA;

    @Value("${sdp.vmsr.sendDeleteReqpool.coresize:40}")
    ...
    public Executor getAsyncExecutor() {
        if (executorA == null) {
            ...
            executor.setThreadNamePrefix("sendDeleteReq-");
            // 拒绝策略
            executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy());
            executor.initialize();
            getLogger().info("sendDeleteReq设置线程池。");
            executorA=executor;
            return executor;
        }else{
            return executorA;
