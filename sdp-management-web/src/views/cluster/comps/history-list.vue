            ...
            <el-table-column
              ...
              <template #default="scope">
                <div style="text-align: center;">
                  <el-button :icon="Delete" text @click="cancelTask(scope.row)"></el-button>
                </div>
              </template>
            </el-table-column>
          </el-table>
...
<script setup>
  ...
  function scalingCountToStr(row){
    if (row.scalingType =='4'){
      ...
      return row.afterScalingCount ? row.afterScalingCount + (row.scalingType == 3 ? 'G' : '台') : ''
    }
  }
  ...
  const filters = ref([
    ...
    },
    {
      ...
      "rules": [],
      "key": "esRuleName",
      "props": {"placeholder": "请输入规则名称", "clearable": true}
    },
    // {
    //   "type": "RemoteSelect",
    //   "label": "任务状态",
  ...
  function cancelTask(item) {
    ElMessageBox.confirm(`您确定要取消当前任务吗？`, '取消任务', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    }).then(() => {
      clusterApi.cancelScalingTask({
        clusterId: item.clusterId,
        taskId: item.taskId,
        groupName: item.groupName
      }).then(res => {
        if (res.result == true) {
          TabelContainer.value.filterEvent(lastFilterData.value);
